...
APP_PATH="./DerivedData/Build/Products/Debug/Second Chance.app"
...
if [ "${1}" == "--test" ] || [ "${1}" == "-t" ]; then
    ...
    if pgrep -f "Second Chance.app" > /dev/null; then
        echo -e "${BLUE}🔄 Closing previous test instances...${NC}"
        pkill -f "Second Chance.app" || true
        sleep 1
    fi
    ...
    if [[ "$STRICT_INSTALL" == true ]]; then
        TEST_MODE=true TEST_DISK_PATH="$TEST_DISK" STRICT_INSTALL=true "$APP_PATH/Contents/MacOS/Second Chance" 2>&1
    else
        TEST_MODE=true TEST_DISK_PATH="$TEST_DISK" "$APP_PATH/Contents/MacOS/Second Chance" 2>&1
    fi
