#!/bin/bash
# test-all-games.sh - Comprehensive integration testing for Nancy Drew game installations
#
...
#   ./test-all-games.sh --quick            # Quick test (skip build)
#   ./test-all-games.sh --no-launch        # Don't launch games, just install
#   ./test-all-games.sh --cleanup          # Delete wrappers after each test
#   ./test-all-games.sh --strict-install   # Fail if silent install fails (no interactive fallback)
#
...
APP_PATH="$SCRIPT_DIR/SecondChance/DerivedData/Build/Products/Debug/SecondChance.app"
TEST_REBUILD_SCRIPT="$SCRIPT_DIR/SecondChance/test-rebuild.sh"
...
cleanup() {
    ...
    pkill -f "test-rebuild.sh" 2>/dev/null || true
...
if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if cd "$SCRIPT_DIR/SecondChance" && xcodebuild -project SecondChance.xcodeproj -scheme SecondChance -configuration Debug -derivedDataPath ./DerivedData > "$BUILD_LOG" 2>&1; then
...
if [[ -n "$SPECIFIC_GAME" ]]; then
    ...
    # Get game slugs by executing Swift script
    GAME_SLUGS_SCRIPT="$SCRIPT_DIR/get-game-slugs.swift"
    
    if [[ ! -f "$GAME_SLUGS_SCRIPT" ]]; then
        log "${RED}❌ get-game-slugs.swift not found${NC}"
        exit 1
    fi
    
    # Execute Swift script to get game IDs in order
    GAMES=()
    while IFS= read -r game_id; do
        GAMES+=("$game_id")
    done < <("$GAME_SLUGS_SCRIPT")
    ...
    log "${GREEN}Found ${#GAMES[@]} games to test${NC}"
fi
...
for game_slug in "${GAMES[@]}"; do
    ...
    # Use test-rebuild.sh with the mounted disk or game directory
    # Use timeout with --foreground and --kill-after to ensure proper cleanup
    # Pass --skip-build since we already built once at the start
    # Pass --strict-install if enabled to prevent fallback to interactive mode
    REBUILD_ARGS=(--skip-build)
    if [[ "$STRICT_INSTALL" == true ]]; then
        REBUILD_ARGS+=(--strict-install)
    fi
    if timeout --foreground --kill-after=5s 600 "$TEST_REBUILD_SCRIPT" "${REBUILD_ARGS[@]}" --test "$test_disk_path" > "$install_log_file" 2>&1; then
        ...
        if [[ -n "$wrapper_app" ]]; then
            ...
            if [[ "$SKIP_LAUNCH" == false ]]; then
                ...
                if timeout "$LAUNCH_TIMEOUT" open "$wrapper_app" 2>&1 | tee "$game_test_dir/launch-log.txt"; then
                    ...
                    # Try to capture a screenshot if the window exists
                    if command -v screencapture &> /dev/null; then
                        screencapture -w "$game_test_dir/screenshot.png" 2>/dev/null || true
                    fi
                    
                    # Kill the game process
                    pkill -f "$(basename "$wrapper_app" .app)" || true
