...
run_unit_tests() {
    ...
    if command -v xcodebuild &> /dev/null; then
        echo xcodebuild test -scheme SecondChance -destination 'platform=macOS' -derivedDataPath "$DERIVED_DATA_PATH" # 2>&1 | \
...
run_integration_tests() {
    ...
    "$SCRIPT_DIR/test-games.sh" "$@"
}

run_quick_test() {
    local game="$1"
    echo -e "${BLUE}⚡ Running quick test for $game...${NC}"
    "$SCRIPT_DIR/test-games.sh" "$game"
}
